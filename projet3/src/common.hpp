...
private:
    void insert_particle(const particle* p) {
        ...
        // Garde : si la subdivision atteint la précision float, les deux
        // particules sont au même endroit (doublon gravitationnel).
        // On arrête la récursion — elles seront traitées comme une seule
        // masse par le nœud parent (comportement BH correct).
        if (nw < std::numeric_limits<float>::min() ||
            nh < std::numeric_limits<float>::min()) return;
