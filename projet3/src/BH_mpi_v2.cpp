// ============================================================
//  BH_mpi_v2.cpp — MPI version 2 : décomposition statique en blocs
//
//  STRATÉGIE : partitionnement statique des particules par index
//  -------------------------------------------------------------
//  Au lieu de décomposer l'espace, on assigne statiquement
//  les particules par tranches d'indices :
//    rang r possède tab[r*chunk .. (r+1)*chunk - 1]
//  Les particules ne migrent JAMAIS d'un rang à l'autre.
//
//  PROTOCOLE PAR ITÉRATION
//  ───────────────────────
//  1. MPI_Allgatherv : même que v1, chaque rang collecte N particules
//     pour construire l'arbre global.
//  2. Construction qtree global (séquentielle).
//  3. calc_interaction seulement sur les particules locales (indice).
//  4. accelerate + move (local).
//  5. MPI_Allgatherv de mise à jour des positions pour la prochaine
//     itération : chaque rang broadcast ses particules (avec nouvelles
//     positions) pour que tous restent synchronisés.
//     → Pas de redistribution : le rang 0 possède toujours les N/P
//     premières particules.
//
//  AVANTAGE vs v1
//  --------------
//  - Pas de redistribution Alltoallv : on sait à l'avance quel rang
//    possède quelle particule (index statique).
//  - Un seul Allgatherv par itération au lieu de Allgatherv + Alltoallv.
//  - Équilibre de charge parfait en nombre de particules (N/P exact).
//
//  INCONVÉNIENT vs v1
//  ------------------
//  - La décomposition ignore la localité spatiale : deux particules
//    proches spatialement peuvent être sur des rangs différents.
//    Cela n'impacte pas la CORRECTION (l'arbre global est identique)
//    mais peut légèrement augmenter les cache-miss dans l'arbre.
//  - Si les particules se regroupent (clustering gravitationnel),
//    certains rangs auront des zones denses → déséquilibre de charge
//    (même nombre de particules mais coût BH très différent).
// ============================================================
...
int main(int argc, char** argv) {
    ...
    for (std::size_t it=0; it<iters; ++it) {
        ...
        // On doit d'abord mettre à jour all_p avec l'état courant
        // des particules locales (positions après move).
#ifdef USE_MPI
...
#else
        // Séquentiel : all_p == local (même vecteur via aliasing)
        // Rien à faire, on reconstruit l'arbre sur all_p directement
        (void)nl;
#endif
