...
public:
    ...
    const qtree* ne() const { return ne_; }
    const qtree* nw() const { return nw_; }
    const qtree* se() const { return se_; }
    const qtree* sw() const { return sw_; }
...
private:
    void insert_particle(const particle* p) {
        ...
        // Garde robuste contre la corruption mémoire à grand N.
        //
        // Problème : avec N=500 000+, le clustering gravitationnel rapproche
        // des particules jusqu'à ce que leurs coordonnées float32 coïncident.
        // La récursion atteignait alors 126 niveaux (jusqu'à FLT_MIN absolu),
        // allouant des millions de nœuds et corrompant le tas (free: invalid ptr).
        //
        // Solution : seuil relatif aux coordonnées de l'espace [-0.5, 0.5].
        // L'epsilon float32 pour des valeurs ~0.5 est ≈ 6e-8.
        // On s'arrête à width < 1e-6, soit ~20 niveaux max, bien avant
        // que les positions des enfants deviennent indiscernables en float32.
        // Les particules co-localisées sont absorbées par le nœud parent
        // comme masse unique — comportement BH physiquement correct.
        static constexpr float MIN_WIDTH = 1e-6f;
        if (nw < MIN_WIDTH || nh < MIN_WIDTH) return;
...
// ── Sérialisation (disponible toujours, MPI ou non) ──────────
